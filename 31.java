...
class People
{
	...
	static void findans(People[] _school,int[] _count) throws IOException
	{
		...
		for(int i=0;i<_count.length;i++)
		{
			if(_count[i]==max)
			{
				...
				{
					bw.write(_school[i].type+" "+_school[i].id+" "+_school[i].sex+" "+
							_school[i].age+" "+_school[i].major+"\r\n");
				}
				else if(_school[i].type.equals("Student"))
				{
					bw.write(_school[i].type+" "+_school[i].id+" "+_school[i].name+" "+_school[i].sex
...
class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		//int n=in.nextInt();
		//int m=in.nextInt();
		FileInputStream is=new FileInputStream("test.in");
		InputStreamReader ir=new InputStreamReader(is);
		BufferedReader in1=new BufferedReader(ir);
		
		String s=in1.readLine();
		int n= Integer.parseInt(s.split(" ")[0]);
		int m= Integer.parseInt(s.split(" ")[1]);
		//String findid= s.split(" ")[1];
